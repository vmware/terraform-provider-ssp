# Releasing terraform-provider-ssp

This document describes how versions of this provider are built, signed, released on GitHub, and
published to the [Terraform Registry](https://registry.terraform.io/providers/vmware/ssp).
...
4. It runs [GoReleaser](https://goreleaser.com/) (`.goreleaser.yml`), which:
   ...
   - Packages each as a `.zip` archive named `terraform-provider-ssp_<version>_<os>_<arch>.zip`.
   - Produces a `terraform-provider-ssp_<version>_SHA256SUMS` checksum file.
   - Attaches a copy of `terraform-registry-manifest.json` to the release, renamed to
     `terraform-provider-ssp_<version>_manifest.json` — this is what tells the registry which
...
[Releases page](https://github.com/vmware/terraform-provider-ssp/releases), open the new draft,
...
2. Install/grant the Terraform Registry's GitHub App access to the
   `vmware/terraform-provider-ssp` repository (a `vmware` org owner action).
...
6. Verify the version appears at
   `https://registry.terraform.io/v1/providers/vmware/ssp/versions` (may take a few minutes).

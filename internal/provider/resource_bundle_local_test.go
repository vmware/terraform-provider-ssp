...
resource "ssp_installer_bundle_local" "test" {
  file_path = %q
}
`, filePath)
}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrSet("ssp_installer_bundle_local.test", "id"),
					resource.TestCheckResourceAttrSet("ssp_installer_bundle_local.test", "package_id"),
					resource.TestCheckResourceAttr("ssp_installer_bundle_local.test", "status", "READY"),
					resource.TestCheckResourceAttr("ssp_installer_bundle_local.test", "version", "1.0.0"),
				),
			},
		},
	})

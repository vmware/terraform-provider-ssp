...
func newLicensesDataSourceMockServer(t *testing.T) *httptest.Server {
	...
	mux.HandleFunc("GET /ssp/licensing-client/licenses", func(w http.ResponseWriter, r *http.Request) {
			...
			"results": []map[string]any{
				{
					...
					"source":                        "VDLS",
					"source_id":                     "nsx-manager-1",
					"sku_code":                      "VDEF-FW-ATP",
					"expiration_date":               1800000000000,
					"expiry_date_with_grace_period": 1800086400000,
				},
			},
		})
	})
...
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("data.ssp_licenses.test", "results.0.license_type", "SUBSCRIPTION"),
					resource.TestCheckResourceAttr("data.ssp_licenses.test", "results.0.source_id", "nsx-manager-1"),
					resource.TestCheckResourceAttr("data.ssp_licenses.test", "results.0.expiry_date_with_grace_period", "1800086400000"),
				),
			},
		},
	})
